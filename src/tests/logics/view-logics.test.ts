...
describe("findMatchingViewLogics", () => {
  ...
  it("should filter out view logics based on obsolete version", () => {
    const viewLogics: ViewLogicConfig[] = [
      {
        name: "Logic 1",
        actionTypes: ["merge"],
        modifiedFields: "all",
    ...
    jest.spyOn(indexUtils._mockable, "getViewLogicConfigs").mockReturnValue(viewLogics);

    const result = findMatchingViewLogics(logicResultDoc, "1.5.0");

    expect(result?.has("Logic 1")).toBe(true);
    expect(result?.has("Logic 2")).toBe(false);
    expect(result?.has("Logic 3")).toBe(false);
    expect(result?.has("Logic 4")).toBe(true);
  });
});
